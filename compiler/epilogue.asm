...
L_code_ptr_bin_apply:
        ; save the last frame postion
        push rbp 
        mov rbp, rsp
        ; set r14 to 0
        mov r14, 0 
        ; get the number of the args and store it in r15
        mov r15, COUNT
        ; decrement r15 to get the number of arguments
        dec r15 
        ; get the last argument and store it in r12
        mov r12, PARAM(r15)
    
        .push_args_of_list_apply:
                ; check if the last argument is equal to null
                cmp r12, sob_nil
                ; if it is, jump to end of pushing list
                je .push_args_of_list_apply_exit
                ; get the first element of the list and store it in r11
                mov r11, SOB_PAIR_CAR(r12)
                ; get the rest of the list and store it in r12
                mov r12, SOB_PAIR_CDR(r12)
                ; push the first element of the list onto the stack
                push r11 
                ; increment r14 to keep track of the number of elements pushed
                inc r14
                ; jump back to the start of the loop to push the next element
                jmp .push_args_of_list_apply

        .push_args_of_list_apply_exit:
                ; set r15 to 0
                mov r15, 0
                ; set r10 to the number of elements pushed
                mov r10, r14
                ; decrement r10 to get the number of elements pushed minus 1
                dec r10 
                
        ; swap arguments in stack from up to down
        .swap_list_args_apply:
                ; check if we have gone over all the args we pushed
                cmp r15, r10
                ; if so, jump to end of swap loop
                jge .swap_list_args_apply_exit
                ; get the argument at index r15 in stack and store it in r12
                mov r12, qword[rsp + 8*r15]
                ; get the argument at index r10 in stack and store it in r11
                mov r11, qword[rsp + 8*r10]
                ; swap between index r12 and index r11
                mov qword[rsp + 8*r15], r11
                mov qword[rsp + 8*r10], r12
                ; increment r15 and decrement r10 for next swap
                inc r15
                dec r10
                ; jump back to the start of the swap loop
                jmp .swap_list_args_apply
                
        .swap_list_args_apply_exit:
                ; set r15 to the number of arguments in frame before pushs
                mov r15, COUNT 
                ; subtract 2 to get the index of the last argument
                sub r15, 2     
                ; set r10 to the number of arguments minus 1
                mov r10, r15 

        .push_args_of_function_apply:
                ; reached last argument
                cmp r15, 0
                je .push_args_of_function_apply_exit

                ; push the argument at index r15
                push PARAM(r15)

                ; decrement r15 to move to the next argument
                dec r15

                ; continue looping
                jmp .push_args_of_function_apply

        .push_args_of_function_apply_exit:
                ; move the total number of arguments to r15
                mov r15, r10

                ; add the number of elements in the list to r15
                add r15, r14

                ; move r15 to r13
                mov r13, r15

                ; push the total number of arguments on the stack
                push r13

                ; get the closure
                mov rax, PARAM(0)

                ; get the environment of the closure
                mov r13, SOB_CLOSURE_ENV(rax)

                ; push the environment of the closure
                push r13
                ...
                ; move the old frame pointer to r11
                mov r11, OLD_RDP

                ; add 5 to r10 to get the number of arguments plus 4
                add r10, 5

                ; add 3 to r15 to get the total number of arguments plus 2
                add r15, 3
        ...
        .shift_stack_apply:
                cmp r12, 0
                jl .shift_stack_apply_exit

                ; move the element at index r12 on the stack to the location specified by r11 on the base pointer
                mov r13, qword [rsp + 8*r12]
                mov qword [rbp + 8*r11], r13

                ; decrement r11 and r12
                dec r11
                dec r12

                ; continue looping
                jmp .shift_stack_apply

        .shift_stack_apply_exit:
                ; increment r10
                inc r10
